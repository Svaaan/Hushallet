...
function PieChartWithCenteredLabels(props: LabelsProps) {
  ...
  const filteredChoreEvents = props.choreEvents?.filter((choreEvent) => {
  ...
  const Labels: React.FC<LabelsProps> = ({ slices }) => {
    return slices.map((slice, index) => {
      ...
      const profile = mockedProfile.find((profile) => profile.id === data.key);
